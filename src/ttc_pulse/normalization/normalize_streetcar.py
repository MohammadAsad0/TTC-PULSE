"""Step 2 streetcar normalization into Silver parquet."""
...
OUTPUT_FILENAME = "silver_streetcar_events.parquet"


def _normalize_streetcar_sql() -> str:
    return """
    WITH gtfs_routes AS (
...
def run_normalize_streetcar(
    db_path: Path | None = None,
    output_path: Path | None = None,
) -> dict[str, object]:
    """Build silver streetcar events parquet from bronze_streetcar."""
    ...
    connection.execute(
        f"CREATE OR REPLACE TEMP TABLE tmp_silver_streetcar_events AS {_normalize_streetcar_sql()}"
    )
    row_count = connection.execute("SELECT COUNT(*) FROM tmp_silver_streetcar_events").fetchone()[0]
    connection.execute(
        f"""
        COPY tmp_silver_streetcar_events
...
def main() -> None:
    result = run_normalize_streetcar()
