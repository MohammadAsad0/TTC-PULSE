"""Raw registry ingestion for TTC bus delay sources."""
...
def discover_bus_files(config_path: Path | None = None) -> dict[str, Any]:
    """Find bus CSV files from schema config."""
    ...
    return {
        ...
        "registry_table": str(config.get("raw_registry_table", "raw_bus_file_registry")),
        "files": [path.resolve() for path in files],
    }


def ingest_bus_registry(
    connection: Any,
    run_id: str,
    ingested_at: str,
    config_path: Path | None = None,
) -> dict[str, Any]:
    """Register bus source files into raw registry storage."""
    paths = resolve_project_paths()
    discovered = discover_bus_files(config_path=config_path)
    ...
    registry_csv_path = paths.raw_root / "bus" / "bus_file_registry.csv"
    if rows:
