"""Raw registry ingestion for TTC streetcar delay sources."""
...
def _resolve_source_root(config: dict[str, Any], workspace_root: Path, project_root: Path) -> Path:
    configured = Path(str(config.get("source_root", "data/streetcar")))
...
def discover_streetcar_files(config_path: Path | None = None) -> dict[str, Any]:
    """Find streetcar CSV files from schema config."""
    paths = resolve_project_paths()
    schema_path = config_path or (paths.configs_root / "schema_streetcar.yml")
...
def ingest_streetcar_registry(
    connection: Any,
    run_id: str,
    ingested_at: str,
    config_path: Path | None = None,
) -> dict[str, Any]:
    """Register streetcar source files into raw registry storage."""
    paths = resolve_project_paths()
    discovered = discover_streetcar_files(config_path=config_path)
    ...
    registry_csv_path = paths.raw_root / "streetcar" / "streetcar_file_registry.csv"
    if rows:
