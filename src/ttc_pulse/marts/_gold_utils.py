...
def create_delay_events_view(
    ...
    resolved_mode_expr = f"COALESCE({mode_expr}, {source_mode_expr}, 'unknown')"

    connection.execute(
        f"""
        CREATE OR REPLACE TEMP VIEW {quote_identifier(view_name)} AS
        SELECT
            {resolved_mode_expr} AS mode,
            ...
            CASE
                ...
                WHEN LOWER({resolved_mode_expr}) = 'bus' AND {min_delay_expr} >= 999 THEN NULL
                WHEN {min_delay_expr} < 0 THEN 0
                ELSE {min_delay_expr}
            END AS min_delay,
            CASE
                WHEN {min_gap_expr} IS NULL THEN NULL
                WHEN LOWER({resolved_mode_expr}) = 'bus' AND {min_gap_expr} >= 999 THEN NULL
