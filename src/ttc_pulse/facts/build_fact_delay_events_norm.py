...
def _create_empty_streetcar_view(connection: duckdb.DuckDBPyConnection) -> None:
    connection.execute(
        """
        CREATE OR REPLACE TEMP VIEW streetcar_events AS
        SELECT
            CAST(NULL AS BIGINT) AS event_id,
            CAST(NULL AS DATE) AS service_date,
            CAST(NULL AS TIMESTAMP) AS event_ts,
...
def _build_fact_sql() -> str:
    ...
    streetcar_rows AS (
        SELECT
            ...
            match_confidence,
            link_status,
            'streetcar' AS source_mode,
            source_file,
            source_sheet,
            source_row_id,
            row_hash,
            ingested_at
        FROM streetcar_events
    ),
    unioned AS (
...
def run_build_fact_delay_events_norm(output_path: Path | None = None) -> dict[str, Any]:
    ...
    streetcar_path = (silver_root / "silver_streetcar_events.parquet").resolve()
    ...
    if streetcar_path.exists():
        connection.execute(
            f"""
            CREATE OR REPLACE TEMP VIEW streetcar_events AS
            SELECT * FROM read_parquet({sql_literal(streetcar_path.as_posix())})
            """
        )
    else:
        _create_empty_streetcar_view(connection)
    ...
    return {
        ...
        "inputs": {
            ...
            "silver_streetcar_events_parquet": streetcar_path.as_posix(),
            "silver_subway_events_parquet": subway_path.as_posix(),
            "bus_exists": bus_path.exists(),
            "streetcar_exists": streetcar_path.exists(),
