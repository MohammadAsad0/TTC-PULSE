...
## Purpose
Confidence gates prevent low-quality mappings from leaking into high-impact stakeholder outputs, especially station rankings, spatial hotspot visualizations, and bus stakeholder views.
...
|---|---|
| Route-level reliability trend | `exact_gtfs_match`, `token_gtfs_match`, `alias_match`, `route_only_match` for GTFS-backed bus routes only |
...
- Bus stakeholder views require a GTFS-backed bus route key; unmatched bus rows stay visible only in QA/review outputs.
...
- Bus severity and regularity calculations treat `min_delay` and `min_gap` values greater than or equal to `999` as invalid sentinels and exclude them from sev/reg aggregation.
