"""Build the gold_route_time_metrics mart."""
...
OUTPUT_FILENAME = "gold_route_time_metrics.parquet"
...
def _build_query() -> str:
    composite_expr = composite_score_sql(
        ...
        partition_columns=["mode", "service_date", "hour_bin"],
        ...
        FROM (
            SELECT
                ...
                SUM(category_count) OVER (
                    PARTITION BY mode, route_id_gtfs, service_date, hour_bin
                ) AS total_count
    ...
    scored AS (
        ...
        LEFT JOIN cause_mix AS c
            ON m.mode = c.mode
            AND m.route_id_gtfs = c.route_id_gtfs
...
def run_build_gold_route_metrics(
    db_path: Path | None = None,
    output_path: Path | None = None,
) -> dict[str, Any]:
    """Build route-level time-sliced reliability metrics with composite scoring."""
    ...
    try:
        ...
        if int(filtered_rows) == 0:
            caveats.append("No route-linked rows with service_date/hour_bin were available.")
...
def main() -> None:
    result = run_build_gold_route_metrics()
