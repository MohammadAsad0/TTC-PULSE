...
## Purpose
Code-first onboarding for teammates: what to read, how the pipeline runs, what DuckDB does here, and how the dashboard consumes the marts.
...
## Execution Order (defined in `pipeline/load_dataset.py`)
1. `run_step1`
2. `build_dimensions`
3. `build_bridge`
4. `build_route_alias`
...
## What DuckDB Does
- load CSVs and register Parquet
- join against GTFS dimensions
- compute grouped metrics, percentiles, window rankings
- fast local analytics without a server

## ETL / Preprocessing
- Step 1 (Bronze): discover sources, write registries, load bronze tables with lineage and schema-drift tolerance.
- Step 2 (Silver): build GTFS dimensions and bridge; normalize route/station/incident; create canonical events and facts; keep review queues for unresolved mappings.
- Step 3 (Gold): build stakeholder-facing marts (route/station/time metrics, rankings, alert validation, linkage quality, spatial).

## Data Model
- Dimensions: `dim_route_gtfs`, `dim_stop_gtfs`, `dim_service_gtfs`
- Alias dimensions: `dim_route_alias`, `dim_station_alias`, `dim_incident_code`
...
## Mode Strategy
- Bus: route-first (location text noisy)
- Subway: station-first (cleaner identity)
- Streetcar: extension scope, not the original contract
...
## Debugging Order
1. source registry / alert manifest
2. Bronze table
3. Silver event
4. fact table
5. Gold mart
